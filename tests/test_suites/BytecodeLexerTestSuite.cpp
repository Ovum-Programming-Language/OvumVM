...
void BytecodeLexerTestSuite::AssertTokenIsEof(const std::vector<ovum::TokenPtr>& tokens, size_t index,
                                               int32_t expected_line, int32_t expected_col) {
  AssertTokenExists(tokens, index);
  ASSERT_EQ(tokens[index]->GetStringType(), "EOF") << "Token at index " << index << " should be EOF";
  ASSERT_EQ(tokens[index]->GetPosition().GetLine(), expected_line)
      << "EOF token should be at line " << expected_line;
  ASSERT_EQ(tokens[index]->GetPosition().GetColumn(), expected_col)
      << "EOF token should be at column " << expected_col;
}

void BytecodeLexerTestSuite::AssertTokenIsNewline(const std::vector<ovum::TokenPtr>& tokens, size_t index,
                                                  int32_t expected_line, int32_t expected_col) {
  AssertTokenExists(tokens, index);
...
void BytecodeLexerTestSuite::AssertTokenIsIdentifier(const std::vector<ovum::TokenPtr>& tokens, size_t index,
                                                      const std::string& expected_value, int32_t expected_line,
  ...
  if (expected_line >= 0) {
    ASSERT_EQ(tokens[index]->GetPosition().GetLine(), expected_line)
        << "Token should be at line " << expected_line;
  }
  if (expected_col >= 0) {
    ASSERT_EQ(tokens[index]->GetPosition().GetColumn(), expected_col)
        << "Token should be at column " << expected_col;
  }
}

void BytecodeLexerTestSuite::AssertTokenIsKeyword(const std::vector<ovum::TokenPtr>& tokens, size_t index,
                                                    const std::string& expected_value, int32_t expected_line,
                                                    int32_t expected_col) {
  AssertTokenExists(tokens, index);
...
void BytecodeLexerTestSuite::AssertTokenIsIntLiteral(const std::vector<ovum::TokenPtr>& tokens, size_t index,
                                                      const std::string& expected_lexeme, int64_t expected_value,
                                                      int32_t expected_line, int32_t expected_col) {
  AssertTokenExists(tokens, index);
  ASSERT_EQ(tokens[index]->GetStringType(), "LITERAL:Int")
      << "Token at index " << index << " should be LITERAL:Int";
...
void BytecodeLexerTestSuite::AssertTokenIsFloatLiteral(const std::vector<ovum::TokenPtr>& tokens, size_t index,
                                                         const std::string& expected_lexeme, double expected_value,
                                                         int32_t expected_line, int32_t expected_col) {
  AssertTokenExists(tokens, index);
...
void BytecodeLexerTestSuite::AssertTokenIsStringLiteral(const std::vector<ovum::TokenPtr>& tokens, size_t index,
                                                          const std::string& expected_raw, const std::string& expected_value,
                                                          int32_t expected_line, int32_t expected_col) {
  AssertTokenExists(tokens, index);
...
void BytecodeLexerTestSuite::AssertTokenIsPunct(const std::vector<ovum::TokenPtr>& tokens, size_t index,
                                                 const std::string& expected_value, int32_t expected_line,
                                                 int32_t expected_col) {
  AssertTokenExists(tokens, index);
